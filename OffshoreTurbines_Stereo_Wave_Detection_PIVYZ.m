...
raw = readimx([image_directory, '\', images(18).name]);
...
%% Resize images to run code faster
...
%%% Remove super bright spots

combined_image_blurred(combined_image_blurred > 65) = nan;

% Canny Edge Deetection
wave_edge = edge(combined_image_blurred, 'Canny', [canny_lower, canny_upper]);
wave_profile = ones(1, nf(2));
...
nexttile()
hold on
contourf(X, Y, combined_image_blurred, 'linestyle', 'none')
